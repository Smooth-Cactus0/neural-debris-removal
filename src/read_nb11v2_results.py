...
with open(os.path.join(base, 'nb11v2_results.json')) as f:
    r = json.load(f)

print('=== nb11v2 L2-SP CORRECTED LAMBDA RESULTS ===')
print()
print(f"{'Config':<22} {'Lambda':>8} {'Supp':>7} {'S_gain':>8} {'Pres':>7} {'P_loss':>8} {'PROXY':>8}")
print('-'*75)
for key, v in r.items():
    lam_str = ('{:.1f}'.format(v['lambda']) if v['lambda'] is not None else '---')
